...
def project_point_cloud_to_image(points: np.ndarray, T_cam_velo: np.ndarray, K_cam: np.ndarray):
    ...
    """
    # points homogeneous [x, y, z, 1]
    pts_hom = np.copy(points)
    pts_hom[:, 3] = 1.0

    # transform to camera frame (3x4) @ (4xN) = (3xN)
    pts_cam = T_cam_velo @ pts_hom.T  # (3, N)

    # filter points behind camera (z <= 0)
    depth = pts_cam[2, :]
    valid = depth > 0  # (N,) boolean mask

    # project to image using intrinsics: K @ pts_cam
    pts_2d = K_cam @ pts_cam  # (3, N)
    ...
    proj_xy = pts_2d[:2, valid].T  # (M, 2)
    depths = depth[valid]           # (M,)

    return proj_xy, depths, valid
...
def build_blueprint() -> bp.Blueprint:
    ...
    cam2_boxes = bp.Spatial2DView(origin="world/cam2", contents=["$origin/boxes_overlay"], name="cam2 Boxes")
    cam3_boxes = bp.Spatial2DView(origin="world/cam3", contents=["$origin/boxes_overlay"], name="cam3 Boxes")

    layout = bp.Horizontal(
        view_3d,
        bp.Vertical(
            bp.Horizontal(cam2_overlay, cam3_overlay, column_shares=[1, 1]),
            bp.Horizontal(cam2_boxes, cam3_boxes, column_shares=[1, 1]),
            row_shares=[1, 1],
        ),
        column_shares=[3, 2],
    )
...
def transform_box_to_cam0(
    ...
    # rotation and translation from T_cam0_velo (4x4)
    R = T_cam0_velo[:3, :3]
    t = T_cam0_velo[:3, 3]

    # transform center: rotate + translate
    center_cam0 = R @ center_velo + t

    # transform rotation: just rotate (no translation for rotation matrices)
    R_box_cam0 = R @ R_box_velo
...
def transform_lidar_to_cam0(pts_velo: np.ndarray, T_cam0_velo: np.ndarray) -> np.ndarray:
    ...
    """
    # ======= STUDENT TODO START (edit only inside this block) =======
    # points homogeneous: (N, 3) -> (N, 4)
    N = pts_velo.shape[0]
    pts_hom = np.hstack([pts_velo, np.ones((N, 1))])  # (N, 4)

    # transform: T_cam0_velo is (4, 4), pts_hom.T is (4, N) -> (4, N)
    pts_cam0 = (T_cam0_velo @ pts_hom.T).T  # (N, 4)
